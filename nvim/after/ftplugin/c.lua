...
if vim.fn.has("win32") == 1 then
	vim.keymap.set("n", "<F5>", ":<C-u>cd %:p:h<CR>:w<CR>te nmake %<.exe<CR>", { buffer = true })
else
	vim.keymap.set("n", "<F5>", ":<C-u>cd %:p:h<CR>:w<CR>make %<<CR>", { buffer = true })
end
...
if vim.fn.has("win32") == 1 then
	if vim.o.shell == "cmd.exe" then
		vim.keymap.set("n", "<f8>", ":<C-u>cd %:p:h<CR>:exec 'term' shellescape(expand('%<')..'.exe')<CR>", { buffer = true })
	else
		vim.keymap.set("n", "<f8>", ":<C-u>cd %:p:h<CR>:exec 'term &' shellescape(expand('%<')..'.exe')<CR>", { buffer = true })
	end
else
	vim.keymap.set("n", "<f8>", ":<C-u>cd %:p:h<CR>:exec 'term' shellescape('./'..expand('%<'))<CR>", { buffer = true })
end
